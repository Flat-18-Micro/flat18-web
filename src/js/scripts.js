...
document.querySelectorAll(".animate-rotate").forEach((parent) => {
    ...
    parent.querySelectorAll(".target").forEach((ele) => {
        ...
        setTimeout(() => { ele.classList.add("playing-animation") }, count * 500)
        setTimeout(() => {
            setInterval(() => {
                ele.classList.remove("playing-animation")
...
for (const ele of document.querySelectorAll(".clickable-target")) {
    ele.addEventListener("click", (e) => {
        let target = false
        if (!e.target.classList.contains("clickable-target")) {
            if(e.target.parentNode.classList.contains("clickable-target")){target = e.target.parentNode}
            else if (e.target.parentNode.parentNode.classList.contains("clickable-target")) { target = e.target.parentNode.parentNode }
            else{return}
        }
        e.stopPropagation()
        if (!target.classList.contains("clicked")) {
            target.classList.add("clicked")
        } else if (!target.classList.contains("single-use")) {
